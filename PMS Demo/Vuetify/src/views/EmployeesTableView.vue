...
export default class EmployeesTableView extends Vue {
    ...
    private jobTitles= [
          ...
          { text: 'SSE', value: 'software engineer' }
        ]
    ...
    // Get all details including the employees in a department
    getDepartment() { 
        this.id = this.$route.params.id;
        DepartmentDataService.getDepartment(this.id)
        .then((response) => {
            if(! response.data){
    ...
    // Load all the details of a selected Departments when the component is mounted
    mounted(){
    ...
    get numberOfPages () {
        return Math.ceil(this.department.employees.length / this.employeesPerPage)
    }
    ...
    // Close the Delete Dialog
      ...
      saveEmployee () {
        if (this.editedIndex > -1) {
          ...
          this.editedEmployee.department = this.department.id;
